...
function enhanceNav() {
    ...
    var idx = h2Content.indexOf(":");
    var first = h2Content.substring(0, idx + 2);
    ...
    for (var i = 0; i < arr.length; i++) {
        if (arr[i]) {
            urlTemp = urlTemp + arr[i] + "/";
            if (urlTemp == pathname) {
                html += "/" + decodeURIComponent(arr[i]) + "";
            } else {
                html += "/<a href='" + urlTemp + "'>" + decodeURIComponent(arr[i]) + "</a>";
            }
        }
    }
    h2.html(html);
        ...
        if (item == '../') {
            ...
            if ($.inArray(fileType, ImageFileType) != -1) {
                ...
                html += '<li title="' + name + '" class="item-image">'
                        + '<a href="javascript:void(0)"><div class="top"><img src="' + url + '?scale=1" data-imgurl="' + url + '" alt="'
                        + name + '"></div>'
